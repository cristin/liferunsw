...
import com.liferun.objects.XMLNode;
...
public class Query {
	...
	public static void performQueryAsync(final QueryMethod queryMethod, final String query, 
			final HttpEntity queryEntity, final Header []headers, final ActionResultDelegate delegate, final QBQueriesDef.QBQueryType queryType){
	...
	public static void authorizeApp(final ActionResultDelegate delegate){
		...
		String signatureParams = String.format("application_id=%s&auth_key=%s&nonce=%s&timestamp=%s",
				QBQueriesDef.APPLICATION_ID, QBQueriesDef.AUTH_KEY, nonce, timestamp);
		String signature = null;
				
		try {
			signature = Signature.calculateHMAC_SHA(signatureParams, QBQueriesDef.AUTH_SECRET);
		...
		formparams.add(new BasicNameValuePair("application_id", QBQueriesDef.APPLICATION_ID));
		...
		formparams.add(new BasicNameValuePair("auth_key", QBQueriesDef.AUTH_KEY));
		...
		Query.performQueryAsync(QueryMethod.Post, QBQueriesDef.GET_AUTH_TOKEN_QUERY, 
				postEntity, null, delegate, QBQueriesDef.QBQueryType.QBQueryTypeGetAuthToken);
	}
}